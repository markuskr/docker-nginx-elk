...
RUN mkdir /downloads
RUN wget -q https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-1.6.0.tar.gz https://download.elasticsearch.org/kibana/kibana/kibana-4.1.1-linux-x86.tar.gz https://download.elasticsearch.org/logstash/logstash/logstash-1.5.2.tar.gz -P /downloads/

RUN mkdir /log
RUN tar xzf /downloads/elasticsearch-1.6.0.tar.gz -C /log
RUN tar xzf /downloads/kibana-4.1.1-linux-x86.tar.gz -C /log
